...
export class TelemetryController {
  ...
  @Get()
  @ApiOperation({
    summary: 'Get aggregated metrics for each vehicle based on the needed time',
  })
  ...
  @ApiOperation({
    summary:
      'Get last telemetry samples for a vehicle based on the needed time',
  })
  @ApiResponse({ status: 200, description: 'Return the telemtry results' })
  //@ApiResponse({ status: 404, description: 'Vehicle not found.' })
  async getVehicleTelemetryData(
    @Param('id') id: number,
    ...
    const tracer = trace.getTracer('telemetry-vehicle');
  ...
  @Get('maps/locations')
  @ApiOperation({
    summary: 'Get latest location for the vehicles',
  })
  ...
  @ApiOperation({
    summary: 'Get locations history for a specific vehicle',
  })
  @ApiResponse({ status: 200, description: 'Return the location results' })
  async getVehicleLocationHistory(@Param('id') id: number) {
    // Prepare query
    const fluxQuery = `from(bucket: "telemetry") \
  |> range(start: -1h) \
  |> filter(fn: (r) => r._measurement == "location") \
  ...
  |> sort(columns: ["_time"], desc: false)`;
    ...
    // Execute query
    const tracer = trace.getTracer('locations-vehicle');
    let result;
    await tracer.startActiveSpan('telemetry-db-operation', async (span) => {
      result = await this.influxDbService.queryData(fluxQuery);
      span.end();
